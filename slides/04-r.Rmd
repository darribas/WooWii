...
```{r}
nl <- read.csv("../data/RR - Netherlands.csv")
```
...
```{r}
library(foreign)
proc <- read.dta("../data/RR-processed.dta")
```
...
* Show a country in which the `Debt1` to `GDP1` ratio has ever been between 20% and 40%
...
```{r}
proc['dtg'] <- 100 * proc$Debt1 / proc$GDP1
debt_to_gdp_clean <- proc[!is.na(proc$dtg), ]
answer <- debt_to_gdp_clean[(debt_to_gdp_clean$dtg > 20) &
                            (debt_to_gdp_clean$dtg < 40), ]
answer[1:5, c('Country', 'Year', 'dtg')]
```

```{r}
unique(answer$Country)
```
...
proc$dgcat <- factor(proc$dgcat.lm, labels = c("0-30%","30-60%","60-90%","Above 90%"),ordered=TRUE)
(RR.correct.mean <- with(proc, tapply( dRGDP, dgcat, mean, na.rm=TRUE )))
```
